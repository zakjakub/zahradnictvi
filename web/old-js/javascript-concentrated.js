...
(function (k) {
    ...
    if (window.StyleFix && window.getComputedStyle) {
        ...
        (function () {
            ...
            var c = {":read-only": null, ":read-write": null, ":any-link": null, "::selection": null}, d = {
                keyframes: "name",
                viewport:  null,
                document:  'regexp(".")'
            };
...
//
//
// Najde správnou metodu pro fullscreen a zavolá ji
function launchFullscreen(element) {
    document.querySelector(".header_link_left").onclick = function () {
        document.querySelector(".header_link_left").onclick = function () {
...
// Najde správnou metodu pro defullscreen a zavolá ji
...
// Vibrations
function vibrate() {
    window.navigator.vibrate([100, 300, 100, 200]);
}

function fbHref(uid) {
    setTimeout(function () {
        window.open("https://fb.com/" + uid, '_blank');
    }, 10);
    window.location = "fb://" + uid;
}
...
function notifyMeIfCookie(message) {
    if (readCookie("messages") != 1) {
        return;
    }
    // Let's check if the browser supports notifications
    if (!("Notification" in window)) {
        alert("Váš prohlížeč bohužel nepodporuje notifikace. Pro přístup k tomuto webu prosím použijte některý z moderních prohlížečů.\n\n" + message);
    }
...
function notifyMeMessage(message) {
    navigator.serviceWorker.register('./sw.js');
    Notification.requestPermission(function (result) {
        if (result === 'granted') {
            navigator.serviceWorker.ready.then(function (registration) {
                registration.showNotification("IS: " + message.toString());
            });
        } else {
            alert("Nemáte povolená upozornění!");
        }
    });
}
...
function scrollListener() {
    var elem = document.querySelector("html header:nth-child(2)");
    var menuHeight = $(document.querySelector("html header:nth-child(2)")).height();
    var threse = (menuHeight > 20) ? menuHeight : 200;
    if ((getScrollTop() - menuHeight) > threse) {
        //document.querySelector("html header:nth-child(2)").style = "position:fixed;top:0;left:0;";
        elem.className = "fixedOnTop";
    } else {
        elem.className = "";
    }
}
...
function openHelp() {
    var win = window.open("help.htm", '_blank');
    win.focus();
}


function submitFormByInput(inputObject) {
    (confirm('Opravdu chcete ODESLAT formulář?') ? $(inputObject.form).submit() : null)
}

function resetFormByInput(inputObject) {
    (confirm('Opravdu chcete SMAZAT formulář?') ? inputObject.form.reset() : null)
}

function notifyClick() {
    notifyMeIfCookie("Clicked!");
}
...
$(document).ready(function () {
    ...
    var raf = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || setTimeout;
...
$(document).ready(function () {
    ...
    //document.addEventListener("mousedown", notifyClick);

// Zpětně změní titulek z elementu někde na stránce.
    if (document.querySelector("#add-meta-title")) {
        document.title = document.querySelector("#add-meta-title").innerHTML;
        if (document.querySelector("#add-meta-title-base")) {
            document.title += " :: " + document.querySelector("#add-meta-title-base").innerHTML;
        }
    }
    ...
    function updateBatteryNotification() {
        if (battery && batteryIndicator) {
            ...
            if (battery.charging) {
                ...
                batteryIndicator.className = "discharged30";
            } else if (!battery.charging && battery.level < 0.4) {
                batteryIndicator.className = "discharged40";
            } else {
                batteryIndicator.className = "notCharging";
            }
            if (!battery.charging && battery.level < 0.2) {
                page.className = "lowbat";
            } else {
                page.className = "";
            }
        }
    ...
    document.addEventListener("keydown", function (e) {
            ...
            var elem = document.querySelector('.editform *:focus');
            if (elem) {
                // CTRL or META + S
                if (e.keyCode === 83 && (navigator.platform.match("Mac") ? e.metaKey : e.ctrlKey)) {
                    e.preventDefault();
                    submitFormByInput(elem);
                }
                // ESC
                if (e.keyCode === 27) {
                    e.preventDefault();
                    resetFormByInput(elem);
                }
            }
        }
        , false);
